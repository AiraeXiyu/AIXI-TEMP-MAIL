...
function buildUrl(endpoint,data={}){
  ...
  if(endpoint.type==='query'){
    const q=encodeQuery(data)
    if(q) url+='?'+q
  }
  return url
}

function buildCurl(endpoint,data){
  let url=buildUrl(endpoint,data)
...
$('copyBase').onclick=async()=>{
  ...
  setButton($('copyBase'),ok?'COPIED ✓':'FAILED')
}

$('copyBaseLarge').onclick=async()=>{
  const ok=await copyText(baseUrl)
  setButton($('copyBaseLarge'),ok?'COPIED ✓':'COPY BASE URL')
}
...
function renderEndpoints(){
    ...
    return `<article class="endpoint">
      ...
      <div class="example-box" id="example-${i}" hidden>
        <div class="example-head">
          ...
          <button class="small-action" data-copy-example="${i}">COPY</button>
        </div>
        <pre>${example}</pre>
      </div>
    </article>`
  ...
  document.querySelectorAll('[data-exec]').forEach(
    b=>b.onclick=()=>openModal(endpoints[Number(b.dataset.exec)])
  )

  document.querySelectorAll('[data-copy-url]').forEach(
    b=>b.onclick=async()=>{
      const ok=await copyText(
        baseUrl+endpoints[Number(b.dataset.copyUrl)].path
      )
      setButton(b,ok?'COPIED ✓':'FAILED')
    }
  )

  document.querySelectorAll('[data-curl]').forEach(
    b=>b.onclick=async()=>{
      const e=endpoints[Number(b.dataset.curl)]
      const ok=await copyText(buildCurl(e,e.example))
      setButton(b,ok?'COPIED ✓':'FAILED')
    }
  )
  ...
  document.querySelectorAll('[data-example]').forEach(
    b=>b.onclick=()=>{
      const box=$('example-'+b.dataset.example)
      box.hidden=!box.hidden
      b.textContent=box.hidden?'EXAMPLE':'HIDE EXAMPLE'
    }
  )

  document.querySelectorAll('[data-copy-example]').forEach(
    b=>b.onclick=async()=>{
      const ok=await copyText(
        pretty(endpoints[Number(b.dataset.copyExample)].example)
      )
      setButton(b,ok?'COPIED ✓':'FAILED')
    }
  )
...
function renderFields(e){
  if(e.type==='none'){
    return '<div class="field"><small>No parameters required for this endpoint.</small></div>'
  }

  return Object.entries(e.schema).map(([key,cfg])=>
    `<div class="field">
      <label>${key}${cfg.required===false?'':' *'}</label>
      <input
        id="field-${key}"
        value="${String(cfg.example??'').replace(/"/g,'&quot;')}"
        placeholder="${String(cfg.example??'').replace(/"/g,'&quot;')}"
      >
...
function collectData(e){
  ...
  if(e.type==='none') return data

  Object.keys(e.schema).forEach(key=>{
...
function openModal(e){
  ...
  $('requestType').textContent=e.type==='none'
    ?'NO PARAMETERS'
    :'QUERY PARAMETERS'
...
$('copyModalUrl').onclick=async()=>{
  const ok=await copyText($('modalUrl').textContent)
  setButton($('copyModalUrl'),ok?'COPIED ✓':'FAILED')
}

$('copyModalExample').onclick=async()=>{
  const ok=await copyText($('modalExample').textContent)
  setButton($('copyModalExample'),ok?'COPIED ✓':'FAILED')
}
...
$('copyCurl').onclick=async()=>{
  ...
  const ok=await copyText(buildCurl(active,data))

  setButton($('copyCurl'),ok?'COPIED ✓':'FAILED')
}
...
$('executeBtn').onclick=async()=>{
  ...
  try{
    ...
    for(const [key,cfg] of Object.entries(active.schema)){
      if(
        cfg.required!==false &&
    ...
    if(active.path==='/api/stream'){
      ...
      es.onerror=()=>{
        response.textContent+='\nStream closed or connection failed.'
        es.close()
      }
    ...
    status.textContent=r.ok
      ...
      :`Request selesai dengan HTTP ${r.status}.`

  }catch(err){
    response.textContent=err.message||'Request gagal.'
    status.textContent='Gagal mengirim request.'
